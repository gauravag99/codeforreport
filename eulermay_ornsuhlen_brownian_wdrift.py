...
def loop(num_sims,ts,t_init, dt,ys,allmotions, gamma, Gamma, mass):
    for j in range(num_sims):
        for i in range(1, ts.size):
            t = t_init + (i - 1) * dt
            y = ys[i - 1]
            ys[i] = y  -gamma*(y-mu) * dt + np.sqrt(Gamma)/mass * np.random.normal(loc=0.0, scale=np.sqrt(dt))
        allmotions[j] = ys
...
print(np.size(ts))
plt.plot(ts,np.zeros(np.size(ts))+mu, 'b-.', lw=0.6,label='Mean velocity')
...
plt.minorticks_on()
plt.grid(which='both')
plt.savefig('stbrownmotwdrift.png',dpi=600)
...
plt.savefig('stdbrown-probdistwdrift')
